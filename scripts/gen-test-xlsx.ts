...
const OUT = join(dirname(fileURLToPath(import.meta.url)), '..', 'pharmapromo-import.xlsx');
...
console.log(`✓ ${OUT}\n  ${rows.length} produits · ${new Set(rows.map(r => r[1])).size} catégories · ${headers.length} colonnes`);
