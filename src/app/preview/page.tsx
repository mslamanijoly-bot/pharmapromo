'use client';
/* Page d'audit visuel (dev) : le MODÈLE UNIQUE rendu sur chaque format × chaque mécanique
   promo, pour capture Playwright. Cas piège volontaire : nom de produit long + litrage,
   et remise -3,99 € (chiffre « moche » qui doit rester lisible partout). */
...
export default function Preview() {
  ...
  return (
    <div style={{ padding: 16, background: '#475569', fontFamily: 'system-ui' }} data-theme="hdf">
      <h2 style={{ color: '#fff', margin: '4px 0 10px' }}>Modèle unique — tous formats × toutes mécaniques</h2>
        ...
        {FORMATS.map(f => (
          <div key={f.id} style={{ display: 'flex', gap: 14, alignItems: 'flex-start', flexWrap: 'wrap' }}>
            ...
            {TYPES.map(t => {
              ...
              return (
                <div key={t} data-cell={`${f.id}-${t}`} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
